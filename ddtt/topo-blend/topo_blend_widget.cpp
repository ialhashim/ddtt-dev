...
topo_blend_widget::topo_blend_widget(topoblend * topo_blend, QWidget *parent) : QWidget(parent), ui(new Ui::topo_blend_widget)
{
    ...
    this->connect(ui->loadCorrButton, SIGNAL(clicked()), SLOT(loadCorr()));
    ...
    this->connect(ui->flipNodeButton, SIGNAL(clicked()), SLOT(flipNode()));
...
void topo_blend_widget::loadCorr()
{
    ...
    for(auto line : data.split("\n"))
    {
        ...
        QMessageBox::information(0,0,items.join("\n"));

        tb->gcoor->addCorrespondences(QVector<QString>() << items.front(), QVector<QString>() << items.back(), -1);
    }
...
void topo_blend_widget::flipNode()
{
    if(!tb->graphs.size()) return;
    Structure::Graph * g = tb->graphs.back();
    ...
    foreach(QListWidgetItem * item, ui->partsList->selectedItems())
    {
        ...
        if(n->type() == Structure::SHEET)
        {
            ...
            NURBS::NURBSRectangled newSurface(ctrlPoint, ctrlWeight, 3, 3, false, false, true, true);
            sheet->surface = newSurface;

            foreach( Structure::Link * l, g->getEdges(n->id) )
            {
                auto coords = l->getCoord(n->id);
                for(auto & c : coords){
                    auto temp = c[1];
                    c[1] = c[0];
                    c[0] = temp;
