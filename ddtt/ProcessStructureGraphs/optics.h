...
public:
    ...
    //struct to store incoming samples in
	struct DataPoint {
        ...
        double reachabilityDistance; // values calculated in OPTICS algorithm
        double coreDistance;         //
    ...
    //constructor
	Optics(double eps, int minPoints, bool fixDimensionMismatches = false ) {
    ...
    //destructor doest have to do anything
	~Optics() {}
	...
	void addSample(std::vector<value_t> & x, size_t label = 0) {
...
private:
	...
	SteepAreas checkPoint(int i, double xi) {
	...
	int findEndOfSteepRegion(int i, SteepAreas updown, double xi) {
		int endPoint = i;
	...
	void tryToMakeACluster(int startUpArea, int endUpArea, std::vector<steepDownArea> &setOfSteepDownAreas, double xi) {

		for (int i = setOfSteepDownAreas.size() - 1; i >= 0; i--) { //move backwards so that the largest cluster comes at the end
			double reachStart = ordered_set[setOfSteepDownAreas[i].start].reachabilityDistance;
			double reachEnd = ordered_set[endUpArea + 1].reachabilityDistance;
			...
			if ( (reachEnd < 0 && reachStart < 0) ||
				...
				EoC = endUpArea;
				double minDistance = fabs(reachEnd - reachStart);
	...
	void updateMibsAndFilterAreas(int index, std::vector<steepDownArea> &setOfSteepDownAreas, double xi, double globalmib) {
	...
	bool reachabilityGreaterThan(double r1, double r2) {
	...
	double maxReachability(double r1, double r2 ) {
		if ((r1 < 0) || (r2 < 0)) return -1;
		return std::max(r1, r2);
	}

	void extractClusters(double xi = 0.02) {
		std::vector<steepDownArea> setOfSteepDownAreas;

		//add a temporary sample at the end with reachability -1
		DataPoint terminator(Eigen::RowVector2d(0, 0), 0);
		terminator.reachabilityDistance = -1;
		ordered_set.push_back(terminator);

		//double xi = 0.02; ///TODO set this from outside. Note: with a lot of data points this should be set lower
		int index = 0;
		double mib = 0;
    ...
    int seedWithSmallestRDistance() {
    	...
    	double smallestReachabilityDistance = data_set[seeds[0]].reachabilityDistance;
    	int seed = 0;
    ...
    void expandSeeds() {
        ...
        for (int j = 0; j < data_set.size(); j++) {  /// TODO: this is the main/only candidate for speedup by indexing
            ...
            double distance = (data_set[i].data - data_set[j].data).norm();
        ...
        if (data_set[i].coreDistance >= 0) { //finally, if we have a core object, add its neighbours to the seed list
			for (int j = 0; j < neighbors.size(); j++) {
				...
				double newReachibilityDistance = std::max(data_set[i].coreDistance, neighbors[j].distance);
