...
void sdf::applyFilter(RichParameterSet *pars)
{
	...
	for(Vertex v : mesh()->vertices()) { maxVal = qMax(m_sdf[v], maxVal); minVal = qMin(m_sdf[v], minVal);}
	for(Vertex v : mesh()->vertices()) m_sdf[v] = (m_sdf[v] - minVal) / (maxVal - minVal);

	for(Vertex v : mesh()->vertices()) drawArea()->drawPoint( points[v], 8, starlab::qtJetColor(m_sdf[v]));
